...
const Header = () => {
  const { user, setPlaces } = useContext(UserContext);
    ...
    <div>
      <header className='flex justify-between'>
        <Link to={"/"} className='text-primary flex items-center gap-1'>
          ...
          <span className='font-bold text-2xl'>RentHub</span>
        </Link>
        <div className='flex gap-2 border border-gray-300 rounded-full py-2 px-4 shadow-md shadow-gray-300'>
          {showSearch ? (
            ...
            <>
              ...
              <div className="border-l border-gray-300"></div>
              <div>Any guests</div>
              <button onClick={handleSearchClick} className='bg-primary text-white px-2 rounded-full'>
